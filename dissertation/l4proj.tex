...
\begin{itemize}
	\item Provide an API which can support a simple single-client and single-server example
	\item Provide an asynchronous, API utilising futures
	\item Expose a simple Preconnection object, which can initiate a connection or begin listening for connections
	\item Expose a simple Connection object, which can send and receive typed data
\end{itemize}

\subsection{Optional}

\begin{itemize}
	\item Provide a message framing mechanism
	\item Expose a Preconnection object, which can only initiate or begin listening if a remote or local endpoint is given, respectively
	\item Expose a Connection object, which can receive partial data
\end{itemize}
...
\begin{itemize}
	\item Provide a simple "self-documenting" API (should be easy to understand uses of API at glance)
	\item Allow applications to provide their own types to be sent (i.e. Serde support)
\end{itemize}

\subsection{Optional}

\begin{itemize}
	\item Provide a minimal performance impact over raw BSD sockets (to be expected with a higher level API)
\end{itemize}
