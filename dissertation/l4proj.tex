...
The \emph{Impl} trait is an primarily an internal trait as it specifies the underlying implementation, but is public to
allow the end user to select from multiple implementations, and create custom implementations.
One method to utilise the \emph{Impl} is to have the methods take either \texttt{&mut self} or \texttt{&self}, and pass
the implementation as a parameter when constructing a \emph{Preconnection}.
...
An alternative method is to have the \emph{Impl} methods not take \texttt{self} in any form, and using it as a marker.
...
Since a \texttt{Future} can take an inderminate length of time to complete, all \emph{Framer}s must have a static
