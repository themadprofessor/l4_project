...
Users of the API will operate exclusively with the abstract types (traits) in order to allow the underlying
...
back to the sender.
A HttpRequest object and HttpResponse object would contain a type which is an Encode type and a Decode type respectively.
...
required.
There exists the Serialize and Deserialize traits from the serde trait, which appear to be similar in use.
The Deserialize trait assumes a
