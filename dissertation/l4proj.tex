...
\begin{lstlisting}[language=Rust, float=h, label=lst:encode, caption={The Encode trait, showing the size_hint method.}]
pub trait Encode {
...
The Rust standard library provides a \texttt{ToSocketAddr} trait which is used to covert types in to a
\texttt{SocketAddr}.
A \texttt{SocketAddr} is an IP address, either IPv4 or IPv6, and a port number.
This trait is implemented for strings by doing a DNS lookup for the domain, if the string is not an IP address.
Since the method in the trait only returns a single \texttt{SocketAddr}, it can not be used for connection racing.
Furthermore, the method is synchronous, so can not be used directly in the API.
Therefore, the \emph{Endpoint} trait is defined to return a \texttt{Vec<SocketAddr>} as shown in Listing
\ref{lst:endpoint}.

\begin{lstlisting}[language=Rust, float=h, label=lst:endpoint, caption={The Endpoint trait.}]
