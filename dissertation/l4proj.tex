...
\usepackage{placeins}[section,above]
...
connections will be operating on.
...
An example is a HTTP framer, which can frame arbitrary data by prepending the data with a HTTP request header, which can
be configured by giving the framer metadata.
A HTTP framer would also deframe arbitrary data be first parsing a HTTP response header.

%\begin{lstlisting}[language=Rust, float, label=lst:conn, caption={An example of the abstract type definition for a
%    Connection object.}]
%trait Connection<F: Framer> {
