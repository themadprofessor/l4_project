...
The runtime is responsible for distributing the Futures among its worker poll if applicable, and ensuring each Future is polled.
...
When an async function reaches an await, the poll being used to progress the Future returns Pending, informing the runtime another Future should be polled.
The Future will continue to return Pending until the Future being awaited completes, at which point the function will continue until the next await or the function returns.
The new syntax allows for asynchronous code to be written quickly and cleanly with no runtime cost, compared to manually handling Futures.
...
Racing will implement the IETF Happy Eyeballs algorithm, in which less-desirable protocol stacks are given a delay before joining the race.
...
While this can result in a large number of connections to race, the use of Happy Eyeballs will reduce the number of connections being raced simultaneously.
