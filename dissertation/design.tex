...
All \emph{Connection} objects are created either directly via \texttt{initiate()} or indirectly via a \emph{Listener}.
...
An \emph{Endpoint} is a type which represents a possible either a local or remote endpoint.
...
As shown in Listing~\ref{lst:endpoint}, \emph{Endpoint} requires a \texttt{resolve} method.
...
implementation for \emph{Endpoint}.
All implementations of \emph{Endpoint} must also be \texttt{Send} since \texttt{resolve} is async, which means it will
...
implements \emph{Endpoint} must exist for the length of the program, hence must have a \texttt{'static} lifetime.
...
If a \emph{Connection} has a \texttt{Drop} implementation, it should have similar semantics to \texttt{close}, but
...
This will ensure the \emph{Connection}s can only handle specific strongly-typed data.
...
An alternative approach is to have the traits generic over the data which the \emph{Connection}s will be operating on.
...
If the \emph{Connection} is generic over the \emph{Framer}, the compiler can ensure only \emph{Connection}s which are
framed with TLS are used, ensuring the data being transmitted is encrypted.
Furthermore, a collection of traits a \emph{Framer} could implement could be created to ensure only \emph{Connection}s
with \emph{Framer}s which have specific properties can be used with certain methods.
