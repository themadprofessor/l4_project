...
A \connection{} then communicates with the network through a \emph{Framer}, until \connection{} is no longer
...
See Section~\ref{subsec:framer} for details on why a \emph{Framer} is required for a \connection{}.
...
For a \emph{Framer} to be given to a \listener{}, a new version of it needs to be given to each \connection{}
received.
As such, \emph{Framer}'s which can be used with the \listener{} trait must implement \texttt{Clone}.
...
If the \connection{} is generic over the \emph{Framer}, the compiler can ensure only \connection{}s which are
framed with TLS are used, ensuring the data being transmitted is encrypted.
Furthermore, a collection of traits a \emph{Framer} could implement could be created to ensure only \connection{}s
